...
function Details() {
    ...
    const handleAddToBasket = (e) => {
        ...
        if ((sizes.length === 0 || selectedSize) && (color.length === 0 || selectedColor)) {
            addToBasket(
                currentProduct.id,
            ...
            alert('Lütfən, mövcud olan seçimləri edin!');
        }
    }
                ...
                <div className='laptop:w-[40%] desktop:w-[60%]'>
                    ...
                    <button
                        ...
                        className={`bg-[#d0c5ad] mini:text-[10px] mob:text-[13px] tablet:text-[16px] my-4 font-bold mini:px-4 tablet:px-6 py-2 rounded-[5px] border hover:bg-[#748371] hover:text-white transition duration-200 tablet:ml-12 laptop:ml-0 
                            ${((sizes.length > 0 && !selectedSize) || (color.length > 0 && !selectedColor)) ? 'opacity-50 cursor-not-allowed' : ''}`}
                        disabled={(sizes.length > 0 && !selectedSize) || (color.length > 0 && !selectedColor)}
                    >
                        Añadir al carrito
                    </button>
